    ...
    if (existingId) {
      ...
      if (cashTotal > 0) {
        ...
        if (existing && existing.linkedIncomeId) {
          Store.updateTransaction(existing.linkedIncomeId, { date: payDate, amount: cashTotal, note: incNote });
        } else {
          const incTx = Store.addTransaction({ date: payDate, type: 'income', amount: cashTotal, category: '股利', note: incNote, source: 'dividend' });
